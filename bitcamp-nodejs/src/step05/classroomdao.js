...
exports.list = (pageNo = 1, pageSize = 3, handler) => {
    ...
    pool.query('select crno, titl, sdt, edt, room from pms2_classroom limit ?, ?',
            [startIndex, pageSize],
...
exports.add = (data, handler) => {
    pool.query(
        'insert into pms2_classroom(crno,titl,sdt,edt,room)\
        values(?, ?, ?, ?, ?)',
        [data.crno, data.titl, data.sdt, data.edt, data.room],
        function(err, result) {
            handler(err, result);
    });
};
...
//http://localhost:8000/classroom/list?pageNo=1&pageSize=50
//
//
//
